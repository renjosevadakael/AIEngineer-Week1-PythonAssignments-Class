...
function App() {
  ...
  const handleFetchFromJira = async () => {
    if (!jiraKey.trim()) {
      setError('Enter a Jira story key (e.g. PROJ-123)')
      return
    }
    ...
    try {
      const issue = await fetchJiraIssue(jiraKey.trim())
      if (!issue) throw new Error('Issue not found or invalid response')
      ...
      <div className="container">
        ...
        <form onSubmit={handleSubmit} className="form-container">
          <div className="form-group">
            <label htmlFor="jiraKey" className="form-label">
              Jira Story Key
            </label>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              >
                {isFetchingJira ? 'Fetching...' : 'Fetch from Jira'}
              </button>
            </div>
          </div>
          <div className="form-group">
